...
const ListItem = (props: {
  ...
  const deleteItem = (item: Item) => {
    props.setItems((prevItems) => prevItems.filter((i) => i.id !== item.id));
  };
  ...
  return (
    <li className="list-row flex flex-col">
      ...
      {editing ? (
        ...
        <div className="flex">
          <span className="p-1 border border-base-100">{props.item.name}</span>
          <a
            className="inline ml-auto rounded"
      ...
      {/* <div className="flex place-items-center gap-2">
        Color:{" "}
        <span
          className={`size-6 rounded-xl border inline-block bg-[${props.item.color}]`}
        ></span>
      </div> */}
      {/* Time */}
      {useTime && (
              ...
              <div key={ix}>
                {editing ? (
                  <span
                    ...
                    onClick={() => {
                      const updatedDays = props.item.days.includes(day)
                        ? props.item.days.filter((d) => d !== day)
                        : [...props.item.days, day];
                      const updatedItem = { ...props.item, days: updatedDays };
                      props.setItems((prevItems) =>
                        prevItems.map((item) =>
                          item.id === props.item.id ? updatedItem : item
                        )
                      );
                    }}
                  >
